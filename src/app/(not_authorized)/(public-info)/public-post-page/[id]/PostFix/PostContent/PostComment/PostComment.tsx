...
type PostCommentPropsType = {
  myProfile: boolean;
  onLikeHandler: () => void;
  onAnswerLikeHandler: (val: number, isLiked: boolean) => void;
...
export const PostComment = ({
  ...
  const [answers, setAnswers] = useState<PostCommentType[]>(
    [] as PostCommentType[]
  );
  ...
  const fetchAnswers = async () => {
    try {
      if (data.postId) {
        const res = await getCommentAnswers(data.postId, data.id);
  ...
  }, [data.postId, data.commentId, submit]);
  return (

...
export async function signInAction(data: SingInData) {
    if (data) {
        ...
        try {
            ...
            if (res.ok) {
                const refreshToken = { refreshToken: getRefreshToken(res.headers.getSetCookie()) };
...
export async function logOutAction(refreshToken: string | undefined) {
    ...
    if (refreshToken) {
        try {
            const res = await fetch(routes.LOGOUT, requestLogoutOptions(refreshToken));
            if (res.ok) {
...
export async function loginGoogleAction(code: string) {
    try {
        const res = await fetch(routes.GOOGLE_LOGIN, requestGoogleLoginOptions(code));
        const responseBody = await res.json();
        if (res.ok) {
...
export async function updateTokenMiddleware(currentRefreshToken: string | undefined) {
    try {
        if (currentRefreshToken) {
            const res = await fetch(routes.UPDATE_TOKENS, requestUpdateTokensOptions(currentRefreshToken));
            const responseBody = await res.json();
            if (res.ok) {

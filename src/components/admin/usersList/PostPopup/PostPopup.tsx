...
export const PostPopup = ({
  ...
  const translate = (key: string): string => t(`Admin.Userlist.${key}`);

  const onEditUser = (key: string) => {
  ...
  return (
    <>
      <div className={s.popup} onClick={() => setShowAreYouSureModal(true)}>
        <div className={s.popup__item} onClick={() => onEditUser('delete')}>
          <svg
          ...
          <p className={s.popup__item__name}>{translate('deleteUser')}</p>
        </div>
        <div className={s.popup__item} onClick={() => onEditUser('ban')}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            ...
            fill="none"
          >
            <g clipPath="url(#clip0_1787_10958)">
              <path
        ...
        <div className={s.popup__item}>
          <Link href={'/admin/profile/' + id} style={{ display: 'flex' }}>
            ...
            <p className={s.popup__item__name} style={{ paddingLeft: '12px' }}>
              {translate('moreInfo')}
            </p>
          </Link>
        </div>
